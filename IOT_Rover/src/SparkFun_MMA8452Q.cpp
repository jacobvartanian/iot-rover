...
//	the sensor. Returns false if communication failed, true if successful.
uint8_t MMA8452Q::init(MMA8452Q_Scale fsr, MMA8452Q_ODR odr)
{
  ...
  // This will manually clock the SCL line if SDA is high to force the MMA8452Q to send data
  // This is necessary after a reset of the I2C master device and power was not removed from the MMA8452Q
  // since the MMA4852Q is still transacting on the I2C bus and expecting the normal signals
/*
...
void MMA8452Q::setupTap(byte xThs, byte yThs, byte zThs, byte timeLimit, byte latency, byte window)
{
  standby();
	...
	writeRegister(PULSE_TMLT, timeLimit);
...
byte MMA8452Q::readTap()
{
	...
	if (tapStat & 0x80) // Read EA bit to check if an interrupt was generated
		return tapStat & 0x7F;
	else
		return 0;
}
...
void MMA8452Q::setupPL()
{
	...
	// writeRegister(PL_COUNT, 0x50);  // Debounce counter at 100ms (at 800 hz) 0x50 = 80, thats 80*1.25 ms Steps
	writeRegister(PL_COUNT, 5);  // Debounce counter at 100ms. Peter set odr to 50. therefore 20ms steps at normal
}
